...
$sql = "SELECT * FROM `wines` WHERE `Nom du mets` LIKE '%".$_GET["plat"]."%'";
$result = $conn->query($sql);

if ($result) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
		array_push($vins, $row["Nom du vin"]);
    }
}
$conn->close();
...
if (empty($vins)){
	...
	array_splice($vins, 5);
	foreach($vins as $vin){
		$url = str_replace('&amp;','&','http://www.bing.com/images/search?q='.urlencode("bouteille vin ".$vin));
		$html = file_get_html($url);
		if(!$html)
			continue;
		
		$images_container = $html->find('div#b_content img');
		
		array_push($imagesUrls, 
			[
				"title"=>urlencode($vin),
				"image_url"=>$images_container[0]->src,
				"buttons"=>[
					[
						"type"=>"web_url",
